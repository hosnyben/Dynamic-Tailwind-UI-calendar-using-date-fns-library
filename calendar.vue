<template>
  <div class="md:grid md:grid-cols-2 md:divide-x md:divide-gray-200">
    ...
    <section class="mt-12 md:mt-0 md:pl-14">
      <h2 class="font-semibold text-gray-900">Schedule for <time :datetime="fullDigitDate" class="capitalize">{{fullDate}}</time></h2>
    ...
    computed: {
      navigation() {
        return this.$store.state.other.navigation
			...
			fullDate() {
				return format(this.currentDay || startOfToday(), 'iiii MMMM Y',{locale: fr})
			},
			fullDigitDate() {
